...
public static class ImpGuessManager
{
    ...
    {
        if (PlayerControl.LocalPlayer != null && Utils.Phantom(PlayerControl.LocalPlayer) && ImmortalManager.immortalAssigned)
        {
            ...
            foreach (var pva in __instance.playerStates.ToArray())
            {
                ...
                var sprite = LoadSprite("BanMod.Resources.image.TargetIcon.png", 100f);
                if (sprite == null)
                {
                ...
                else
                {
                    ...
                    {
                        Logger.Info("?? SpriteRenderer non trovato nel bottone.");
                    }
                }
                ...
                button.OnClick.AddListener((UnityAction)(() =>
                {
                    ...
                    if (PlayerControl.LocalPlayer.Data.IsDead || ImpguesserUI != null )
                        return;
                    // Chiama il metodo per gestire il click del bottone "Guesser"
                    ImpGuesserOnClick(pva.TargetPlayerId, __instance);
                }));
            }
        }
    }

    static void ImpGuesserOnClick(byte playerId, MeetingHud __instance)
    {
        if (AmongUsClient.Instance.AmHost)
        {
            ...
            if (pc == null || !pc.IsAlive() || ImpguesserUI != null ) return;
    ...
    static void ShowHostConfirmationUI(byte playerId, MeetingHud __instance)
    {
        ...
        ImpguesserUI = canvasGO; // Salva il riferimento al canvas per distruggerlo in seguito
        ...
        System.Action value = () =>
        {
            ...
            bool isImmortal = ImmortalManager.ImmortalPlayers.Contains(playerId);
            ...
            Object.Destroy(ImpguesserUI); // Distruggi l'UI di conferma
            ImpguesserUI = null;
        };
        yesButton.onClick.AddListener((UnityAction)value);

...
namespace BanMod
{
    static class Logger
    {
        ...
        {
            if (!IsEnable || DisableList.Contains(tag) || (level == LogLevel.Debug)) return;
            var logger = BanMod.Logger;
        ...
        {
            // Implementazione del metodo Info
            // Log su Unity o su un file di log

            UnityEngine.Debug.Log(v);  // Usa Unity Debug Log per la console di Unity

            // Scrittura su un file di log (opzionale)
            string logFile = "log.txt";
            File.AppendAllText(logFile, $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} - {v}\n");
        }

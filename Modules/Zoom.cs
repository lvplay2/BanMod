...
public static class Zoom
{
    ...
    public static void Postfix()
    {
        try
        {
            ...
            if (Options.EnableZoom.GetBool() && !GameStates.IsMeeting && GameStates.IsCanMove && PlayerControl.LocalPlayer.Data.IsDead || Options.EnableZoom.GetBool() && GameStates.IsLobby && GameStates.IsCanMove)
            {
                ...
                switch (Input.mouseScrollDelta.y)
                {
                    ...
                    case < 0:
                        {
                            if (Options.EnableZoom.GetBool() && GameStates.IsDead || Options.EnableZoom.GetBool() || Options.EnableZoom.GetBool() && GameStates.IsLobby)
                            {
